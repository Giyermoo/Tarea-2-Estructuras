...
#define NUM_RECORDS 1000 //cantidad de lineas que tendra input.csv
#define MAX_CITY_NAME 50 //largo max nombre de ciudad
#define MAX_CITIES 100 //numero maximo de ciudades distintas que se puede leer desde cities.txt


//Abre cities.txt, lo lee linea por linea y carga las ciudades en el arreglo cities
void load_cities(const char *filename, char cities[][MAX_CITY_NAME], int *num_cities) {
    ...
    *num_cities = 0;
    ...
    fclose(file);
} //Guarda el número total de ciudades leídas en *num_cities
...
void generate_random_csv(const char *filename, char cities[][MAX_CITY_NAME], int num_cities) {
    ...
    //bucle para generar 1000 registros
...
int main() {
    ...
    return 0;
} //Carga las ciudades desde cities.txt y llama a generate_random_csv() para generar el archivo final.